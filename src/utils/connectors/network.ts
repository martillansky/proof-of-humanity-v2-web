...
import { FALLBACK_CHAIN, RPC } from "constants/chains";
...
class MiniRpcProvider implements AsyncSendable {
      ...
      if (b.request.method === "wallet_switchEthereumChain") {
        try {
          this.connector.changeChainId(
            parseInt((b.request.params as [{ chainId: string }])[0].chainId)
          );
  ...
  public readonly sendAsync = async (
    request: {
      jsonrpc: "2.0";
  ...
  public readonly request = async (
    ...
    return new Promise((resolve, reject) => {
      this.batch.push({
        request: { jsonrpc: "2.0", id: this.nextId++, method, params },
        resolve,
        reject,
      });
    });
...
export class NetworkConnector extends AbstractConnector {
  ...
  constructor({ urls, defaultChainId }: NetworkConnectorArguments) {
    if (!defaultChainId && Object.keys(urls).length !== 1)
      throw new Error("defaultChainId is a required argument with >1 url");
    ...
    this.providers = Object.keys(urls).reduce<{
      ...
      (acc, chainId) => ({
        ...acc,
        [Number(chainId)]: new MiniRpcProvider(
          this,
          Number(chainId),
          urls[Number(chainId)]
        ),
