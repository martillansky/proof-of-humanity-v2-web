...
export default withClientConnected<FormProps & JSX.IntrinsicAttributes>(function Form({
  contractData,
  totalCosts,
  renewal,
}) {
  ...
  state$.onChange(({ value }) => {
    ...
    const funded = selfFundedWei > totalCosts[chainId] ? totalCosts[chainId] : selfFundedWei;
    if (renewal)
  ...
  if (
    !isConnected ||
    (renewal && (renewal.claimer.id !== address!.toLowerCase() || renewal.chain.id !== chainId))
  )
    return <Connect renewalAddress={renewal?.claimer.id} renewalChain={renewal?.chain} />;
  ...
  return (
    <>
            ...
            {steps.map((item, i) => (
              <Fragment key={i}>
                <div className="m-1 flex items-center">
                  <div
                    className={cn('centered h-6 whitespace-nowrap rounded-full text-sm', {
                      'w-6 border border-slate-200 font-bold text-slate-400': step$.get() < i,
                      'gradient px-2 font-bold uppercase text-white': step$.get() === i,
                      'gradient w-6 cursor-pointer font-bold text-white': step$.get() > i,
                    })}
                ...
                {i !== steps.length - 1 && (
                  <div
                    className={cn('h-px w-full', step$.get() > i ? 'gradient' : 'bg-slate-200')}
                  />
                )}
              </Fragment>
            ))}
      ...
      <Switch value={step$}>
        {{
          [Step.info]: () => <InfoStep advance={() => step$.set(Step.photo)} state$={state$} />,
          [Step.photo]: () => (
            <PhotoStep advance={() => step$.set(Step.video)} photo$={media$.photo} />
          ),
          [Step.video]: () => (
            <VideoStep
          ...
          [Step.finalized]: () => (
            <Finalized requiredVouches={contractData[chainId].requiredNumberOfVouches} />
          ),
        }}
      </Switch>
    </>
