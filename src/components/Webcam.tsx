...
const Webcam: React.FC<WebcamProps> = ({
  ...
  const switchFacingMode = () => setFacingMode(facingMode === 'user' ? 'environment' : 'user');
  ...
  return (
    <div className="relative">
      <ReactWebcam
        ...
        videoConstraints={{
          width: IS_MOBILE ? { min: 640, exact: 1280 } : { min: 640, ideal: 1920 },
          height: IS_MOBILE ? { min: 480, exact: 720 } : { min: 480, ideal: 1080 },
      ...
      {recording ? (
        ...
        <>
          {IS_MOBILE && devices.length > 1 && (
            <CameraButton secondary className="left-4 top-4" onClick={switchFacingMode}>
              <FlipCameraIcon className="h-8 w-8 fill-white" />
            </CameraButton>
          )}
          <CameraButton
            secondary
            className={cn('left-4', IS_MOBILE && devices.length > 1 ? 'top-20' : 'top-4')}
            onClick={() => setMirrored((o) => !o)}
          >
            <MirrorIcon className="h-8 w-8 fill-white" />
          </CameraButton>
          <CameraButton secondary className="right-4 top-4" onClick={toggleFullscreen}>
            <FullscreenIcon className="h-8 w-8 fill-white" />
          </CameraButton>
        </>
      )}

      <CameraButton className="bottom-8 left-1/2 -translate-x-1/2 opacity-90" onClick={action}>
